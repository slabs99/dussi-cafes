...
function CoffeeLoveIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="-0.79 0 110 110" xmlns="http://www.w3.org/2000/svg" fill="currentColor" aria-hidden>
        ...
        <div className="px-5 pt-4 pb-5">
          ...
          {cafe.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {cafe.categories.map((cat) => (
                <span
                  key={cat}
                  className="text-[0.65rem] uppercase tracking-wider border border-[#E0DDD9] rounded-sm px-2 py-0.5 text-stone-600"
                >
          ...
          <div className="flex gap-3 mt-5">
            <a
              ...
              className="flex-1 bg-[#2D6A4F] text-white text-xs uppercase tracking-widest py-2.5 rounded-lg text-center hover:bg-[#245a42] transition-colors"
            >
              {t.openInMaps}
            </a>
            <button
              onClick={onNext}
              className="flex-1 border border-[#E0DDD9] text-xs uppercase tracking-widest py-2.5 rounded-lg text-stone-600 hover:border-stone-400 hover:text-stone-900 transition-colors"
            >
              {t.tryAnother}
            </button>
          </div>
